-- upstream: https://github.com/graphql/graphql-js/blob/7b3241329e1ff49fb647b043b80568f0cf9e1a7c/src/language/predicates.js
...
local function isDefinitionNode(node): boolean
	return
		isExecutableDefinitionNode(node) or
		isTypeSystemDefinitionNode(node) or
		isTypeSystemExtensionNode(node)
end

function isExecutableDefinitionNode(node): boolean
	return
		node.kind == Kind.OPERATION_DEFINITION or
		node.kind == Kind.FRAGMENT_DEFINITION
end

function isSelectionNode(node): boolean
	return
		node.kind == Kind.FIELD or
		node.kind == Kind.FRAGMENT_SPREAD or
		node.kind == Kind.INLINE_FRAGMENT
end

function isValueNode(node): boolean
	return
		node.kind == Kind.VARIABLE or
		node.kind == Kind.INT or
		node.kind == Kind.FLOAT or
		node.kind == Kind.STRING or
...
function isTypeNode(node): boolean
	return
		node.kind == Kind.NAMED_TYPE or
		node.kind == Kind.LIST_TYPE or
		node.kind == Kind.NON_NULL_TYPE
end

function isTypeSystemDefinitionNode(node): boolean
	return
		node.kind == Kind.SCHEMA_DEFINITION or
		isTypeDefinitionNode(node) or
		node.kind == Kind.DIRECTIVE_DEFINITION
end

function isTypeDefinitionNode(node): boolean
	return
		node.kind == Kind.SCALAR_TYPE_DEFINITION or
		node.kind == Kind.OBJECT_TYPE_DEFINITION or
...
function isTypeExtensionNode(node): boolean
	return
		node.kind == Kind.SCALAR_TYPE_EXTENSION or
		node.kind == Kind.OBJECT_TYPE_EXTENSION or
